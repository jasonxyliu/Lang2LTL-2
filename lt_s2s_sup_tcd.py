...
S2S_MODELS = T5_MODELS.extend(["pt_transformer"])
...
#     parser.add_argument("--data_fpath", type=str, default="data/holdout_split_batch12_perm/symbolic_batch12_perm_utt_0.2_0.pkl", help="complete file path or prefix of file paths to train test splits.")
#     parser.add_argument("--model_dpath", type=str, default=None, help="directory to save model checkpoints.")
#     parser.add_argument("--model_name", type=str, default="t5-large", choices=S2S_MODELS, help="name of supervised seq2seq model.")
#     parser.add_argument("--checkpoint", type=str, default=None, help="checkpoint to use for inferance.")
#     parser.add_argument("--results_dpath", type=str, default=None, help="directory to save evaluation results.")
...
#                         format='%(message)s',
#                         handlers=[
#                             logging.FileHandler(f'{args.results_dpath}/s2s-tcd_{args.model_name}_{ckpt_dname}_{Path(args.data_fpath).stem}.log', mode='w'),
...
#     else:  # prefix of file paths, e.g. for all utterance holdout datasets: data/holdout_split_batch12_perm/symbolic_batch12_perm_utt
...
#         # Load trained model
#         if "t5" in args.model_name or "bart" in args.model_name:  # pretrained T5/Bart from Hugging Face
#             model_dpath = os.path.join(args.model_dpath, ckpt_dname)
#             logging.info(f"Load model and checkpoint: {model_dpath}")
#             s2s = Seq2Seq(model_dpath, args.model_name)
#         else:
#             raise TypeError(f"ERROR: unrecognized model, {args.model_name}")
#         logging.info(f"Number of trainable parameters in {args.model_name}: {count_params(s2s)}")
...
#         result_log_fpath = f"{args.results_dpath}/s2s-tcd_{args.model_name}_{ckpt_dname}_{Path(data_fpath).stem}_log.csv"
#         analysis_fpath = f"{os.path.expanduser('~')}/data/shared/lang2ltl/data/analysis_symbolic_batch12_perm.csv"
#         acc_fpath = f"{args.results_dpath}/s2s-tcd_{args.model_name}_{ckpt_dname}_{Path(data_fpath).stem}_acc.csv"
#         evaluate_sym_trans(s2s, data_fpath, result_log_fpath, analysis_fpath, acc_fpath, batch_size=50)
