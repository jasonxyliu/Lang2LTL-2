...
from srer import run_srer
from reg import run_reg
from spg import run_spg
from lt import run_lt
...
if __name__ == "__main__":
    ...
    run_srer(utt_fpath, srer_out_fpath)

    # Referring Expression Grounding (REG)
    run_reg(srer_out_fpath, graph_dpath, osm_fpath, args.topk, args.ablate, reg_out_fpath)

    # Spatial Predicate Grounding (SPG)
    run_spg(reg_out_fpath, graph_dpath, osm_fpath, args.topk, rel_embeds_fpath, spg_out_fpath)

    # Lifted Translation (LT)
    run_lt(spg_out_fpath, model_fpath, lt_out_fpath)
