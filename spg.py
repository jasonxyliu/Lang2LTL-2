...
from pyproj import Transformer  # convert GPS to Cartesian coordinates: https://stackoverflow.com/a/69604627
import utm
...
def align_coordinates(graph_dpath, osm_landmarks, spot_waypoints, coord_alignment, crs):
    ...
    offset = 0  # translation needed to be added to GPS coordinates from OSM (default: 0, not needed if no robot graph)
    ...
    if not graph_dpath or not spot_waypoints:
        ...
        list_waypoints = [Path(image_fpath).stem for image_fpath in os.listdir(os.path.join(graph_dpath, "images"))]
        ...
        for W in spot_waypoints:
            ...
            if is_landmark:
                ...
                spot_coordinate = np.array([spot_waypoints[W]["position"]["x"], spot_waypoints[W]["position"]["y"]])
...
def fake_spot_waypoints(obj_fpath, crs):
    """
    Create a "fake" robot graph of objects similar to that created by Spot GraphNav.
    """
    objects = load_from_file(obj_fpath)

    # Set the origin of the coordinates to the location of the robot
    robot = None
    ...
    # Convert landmarks locations from geographic coordinates to Cartesian coordinates
...
def load_lmks(graph_dpath=None, osm_fpath=None):
    ...
    try:
        ...
        waypoints, transformer = fake_spot_waypoints(os.path.join(graph_dpath, "obj_locs.json"), crs=None)
    else:
    ...
    if os.path.isfile(osm_fpath):
        ...
        # Use GPS coordinates of first landmark to get a zone number for UTM conversion
        lmk = list(osm_landmarks.keys())[0]
        _, _, zone, _ = utm.from_latlon(osm_landmarks[lmk]["lat"], osm_landmarks[lmk]["long"])
    ...
    alignment_lmks = []
    alignment_fpath = os.path.join(graph_dpath, "alignment.json")  # contain Spot graph waypoints in GPS coordinates
    if os.path.isfile(alignment_fpath):
        alignment_lmks = load_from_file(alignment_fpath)

    # Put Spot waypoints and OSM landmarks in the Cartesian coordinates
    landmarks = align_coordinates(graph_dpath, osm_landmarks, waypoints, alignment_lmks, transformer)
