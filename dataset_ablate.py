...
def split_true_lmk_grounds(loc, lmks_fpath, obj_fpath, sp_fpath, res_fpath):
    ...
    res = {"text": defaultdict(lambda: defaultdict(list)), "image": defaultdict(lambda: defaultdict(list)), "both": defaultdict(lambda: defaultdict(list))}

    for lmk, grounds in lmk_grounds.items():
        for ground in grounds:
            if "*" in ground:  # unique referring expression can identify landmark without anchor
                modal = "text" if lmk in lmk_ids else "image"
                res[modal][lmk]["proper_names"].append(ground["*"])
                if lmk not in sp_grounds[modal]["None"]:
                    sp_grounds[modal]["None"].append(lmk)
            elif "@" in ground:  # ambiguous referring expression if used without anchor
                modal = "text" if lmk in lmk_ids else "image"
                res[modal][lmk]["generic_names"].append(ground["@"])
                ...
                if ground[rel] not in sp_grounds[modal][rel]:
                    sp_grounds[modal][rel].append(ground[rel])

    save_to_file(sp_grounds, sp_fpath)
    save_to_file(res, res_fpath)


def generate_dataset(ltl_fpath, obj_fpath, osm_fpath, sp_fpath, res_fpath, utts_fpath, outs_fpath, nsamples, seed):
    """
...
if __name__ == "__main__":
    ...
    # img_dpath = os.path.join(data_dpath, "maps", LOC2GID[args.loc], "images")
    obj_fpath = os.path.join(data_dpath, "maps", LOC2GID[args.loc], "obj_locs.json")
    ...
    sp_fpath = os.path.join(loc_dpath, f"{args.loc}_sp_grounds_ablate.json")
    res_fpath = os.path.join(loc_dpath, f"{args.loc}_res_ablate.json")
    utts_fpath = os.path.join(loc_dpath, f"{loc_id}_utts_ablate.txt")
    outs_fpath = os.path.join(loc_dpath, f"{loc_id}_true_results_ablate.json")
    ...
    #     generate_dataset(ltl_fpath, obj_fpath, osm_fpath, sp_fpath, res_fpath, utts_fpath, outs_fpath, args.nsamples, args.seed)
