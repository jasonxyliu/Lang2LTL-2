...
class RecordingInterface(object):
    ...
    def __init__(self, robot, download_filepath, client_metadata):
        ...
        self._download_filepath = os.path.join(download_filepath, f'downloaded_graph_{timestr}')
        ...
        self._pixel_format = dict(image_pb2.Image.PixelFormat.items()).get("PIXEL_FORMAT_RGB_U8")
        self._image_dpath = os.path.join(self._download_filepath, "images")
        os.makedirs(self._image_dpath, exist_ok=True)
    ...
    def _create_default_waypoint(self, *args):
        ...
        if resp.status == recording_pb2.CreateWaypointResponse.STATUS_OK:
            # Get hand color image at current waypoint and save it locally
            image_request = [build_image_request("hand_color_image", pixel_format=self._pixel_format)]
            ...
            current_waypoint_id = self._graph_nav_client.get_localization_state().localization.waypoint_id
            image_fpath = os.path.join(self._image_dpath, f"{current_waypoint_id}.jpg")
            cv2.imwrite(image_fpath, img)
    ...
    def run(self):
        ...
        while True:
            ...
            (8) Create new edges between parent and child waypoints using odometry.  # new feature
            (9) Create new edges between default waypoints using odometry.  # new feature
